 ...
 */
@WebServlet(name = "ShoppingServlet", urlPatterns = { "/products",
...
public class ShoppingServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		products.add(new Product(1, "Bravo 32' HDTV",
				"Low-cost HDTV from renowned TV manufacturer", 159.95F));
		...
		products.add(new Product(3, "Bravo Stereo System",
				"5 speaker hifi system with iPod player", 129.95F));
	...
	private void showCart(HttpServletRequest request,
		...
		if (cart != null) {
			...
			writer.println("<tr><td colspan='4' " + "style='text-align:right'>"
					+ "Total:" + currencyFormat.format(total) + "</td></tr>");
			writer.println("</table>");
		}
	...
	private void sendProductDetails(HttpServletRequest request,
		...
		if (product != null) {
			...
			writer.println("<input type='hidden' name='id' " + "value='"
					+ productId + "'/>");
			writer.println("<table>");
			writer.println("<tr><td>Name:</td><td>" + product.getName()
					+ "</td></tr>");
